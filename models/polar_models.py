...
from models.sc_models import CheckNodeVanilla, BitNodeVanilla, CheckNodeTrellis, BitNodeTrellis, \
    BitNodeNNEmb, CheckNodeNNEmb, Embedding2LLR, Embedding2LLRTrellis, EmbeddingY, EmbeddingX, hard_dec, BatchNormModel, MyPolar5GEncoder,InterleavingMethod, EyModel, MyPreDecoder
...
class SCDecoder(Model):
    ...
    def RecoverNeuralRateMatching(self, ey, N, bits_size, sorted_indices=[]):
        ...
        ex = self.Ex((batch, N//self.channel.BPS))
        #ex = self.Ex((batch, N))
        ...
        if E_size > N:
            ...
            ey_temp = self.Ex((batch, N // self.channel.BPS))
    ...
    def forward_design(self, batch, N, ebno_db=None):
        ...
        no = tf.ones((batch, y.shape[1], 1)) * self.channel_design.no
...
class system_model(tf.keras.Model):
    def __init__(self, model, mc_length, code_rate, batch, tol, Ns, design_path, design_load, mc_design, load_nsc_path=None, design5G=False):
        super().__init__()
        self.batch = batch
...
class SCNeuralDecoder(SCDecoder):
    ...
    def __init__(self, channel, embedding_size, hidden_size, layers_per_op,
                 activation='elu', batch=100, eyN0=False, trained_block_norm=10, *args, **kwargs):
...
class SCListDecoder(SCDecoder):
    ...
    def forward_eval(self, batch, N, info_indices, frozen_indices, A, Ac, ebno=None):
        ...
        u, x, llr_u1_enc = self.encode(ex_enc, f_enc, N, r, sample=True)
        # if self.encoder5g:
        #     b_info = tf.gather(params=u,
        #                             indices=A,
        #                             axis=1)
        #     self.encoder5g = Polar5GEncoder(k=A.shape[0], n=self.channel.E * self.channel.BPS, verbose=False,
        #                                       channel_type='uplink')
        #     x = self.encoder5g(b_info[...,0])[...,None]
        #     self.pre_decoder = MyPreDecoder(self.encoder5g, dec_type='SCL', list_size=16)
        ...
        if self.pre_decoder:
            eyx = self.pre_decoder.call(eyx[...,0])[...,None]
            eyx = tf.gather(eyx, self.bit_reversed_indices, axis=1)
...
class SCNeuralListDecoder(SCListDecoder, SCNeuralDecoder):
    ...
    def __init__(self, channel, embedding_size, hidden_size, layers_per_op, eyN0=False,
                 activation='elu', batch=100, list_num=4, crc=None, trained_block_norm=10, *args, **kwargs):
        ...
        SCNeuralDecoder.__init__(self, channel=channel, batch=batch, embedding_size=embedding_size,
                                 ...
                                 trained_block_norm=trained_block_norm, eyN0=eyN0, *args, **kwargs)
...
class SCLsionnaDecoder(SCListDecoder):
    ...
    def list_decoder_with_5g(self, ey, batch, N, info_indices, A):
        ey = self.pre_decoder.call(ey[...,0])[...,None]
        #ey = tf.gather(ey, self.bit_reversed_indices, axis=1)
        bits_hat = self.decoder_sc(ey[..., 0])[...,None]  # this is needed to initialize the decoder
...
class NeuralPolarDecoder(SCNeuralDecoder):
    def __init__(self, channel, embedding_size, hidden_size, layers_per_op,
                 ...
                 input_distribution='sc', input_state_size=2, pred_decay=None, lr_decay=None, trainEyOnly=False, eyN0=False, EyPerSnr=False):
        SCNeuralDecoder.__init__(self, channel, embedding_size, hidden_size, layers_per_op, activation, eyN0=eyN0, batch=batch)
    ...
    #@tf.function
    def estimation_step(self, batch, N, train_ex=False, full_bu_depth=None):
        x, _ = self.sample_inputs(batch, N)
    ...
    def design(self, train_block_length=8, train_batch=10, num_iters=10000, load_nsc_path=None, full_bu_depth=None):
        if load_nsc_path is not None:
            self.load_model(load_nsc_path)
        res = dict()
        for snrdb in np.arange(0, 5, 0.5):
            n = int(np.log2(train_block_length))
            errors = dict()
            ce = dict()
            sorted_bit_channels =dict()
            for i in range(6, n+1):
                errors[int(2**i)]= np.zeros(shape=(2**i), dtype=dtype)
                ce[int(2**i)] = np.zeros(shape=(2**i), dtype=dtype)

            for k in tqdm(range(1,num_iters)):
                v_array, pred_y_array, loss_y_array  = self.design_step(train_batch, train_block_length, full_bu_depth=full_bu_depth, snrdb=snrdb)
                for i in range(6, n+1):
                    pred = pred_y_array[i]
                    u = v_array[i]
            ...
            for i in range(6, n+1):
                errors[int(2**i)]/= k
                ce[int(2**i)] /= k
                sorted_bit_channels[int(2**i)] = np.argsort(ce[int(2**i)])
                # print('')
                # print(f'rate 1e-3, N={2**i}', np.where(np.cumsum(errors[int(2**i)][sorted_bit_channels[int(2**i)]]) <= 1e-3)[0][-1])
            res[snrdb] = [errors.copy(), ce.copy(), sorted_bit_channels.copy()]

        save_name = f'design_all_{wandb.config["group"]}_bps{self.channel.BPS}_N{train_block_length}_mc{num_iters*train_batch:.0e}'.replace('+','')
        save_tmp_path = f"./artifacts/tmp/{wandb.run.name}/design.npy"
        # Extract the directory part of the path
        directory = os.path.dirname(save_tmp_path)
        if not os.path.exists(directory):
        ...
        weights_artifact = wandb.Artifact(save_name, type='design_all')
    ...
    def train(self, train_block_length=8, train_batch=10, num_iters=10000,
              ...
              logging_llr_hist=False, full_bu_depth=None):
        if load_nsc_path is not None:
            self.load_model(load_nsc_path)
            if self.trainEyOnly:
        ...
        for k in tqdm(range(1,num_iters)):
            train_ex = (k % 10 == 0)
            r = self.estimation_step(train_batch, train_block_length, train_ex=train_ex, full_bu_depth=full_bu_depth)
