...
class SingleCarrier(Model):
    ...
    def __init__(self,
        ...
        if self.channel_model == "tdl":
            self.channel = self.channel_time_tdl
        elif self.channel_model == "awgn":
            self.channel = self.channel_awgn

    @tf.function() # enable graph mode for increased throughputs
    def channel_time_tdl(self, s, no):
        batch_size = tf.shape(s)[0]
        num_time_steps = s.shape[-1] + self.l_tot - 1
        a, tau = self.tdl(batch_size,  num_time_steps, self.bandwidth)
        h_time = cir_to_time_channel(self.bandwidth, a, tau, self.l_min,
                                     self.l_max, True)
        y = self._apply_channel(s, h_time)  # apply channel
        y = self.channel_noise(y, no)  # add noise
        return y

    def channel_awgn(self, s, no):
        y = self.channel_noise(s, no)  # add noise
        return y
    ...
    def sample_channel_outputs(self, c, ebno_db):
        ...
        self.no = no
        c = tf.squeeze(c) # shape (batch_size, frame_size)
        # map bits to symbols
        s = self.mapper(c)
        s = s[:,None, None]
        ...
        # remove channel delay extension
        y = y[..., :self.frame_size]

        # reshape to real-valued tensor
...
if __name__ == "__main__":
    ch = SingleCarrier(num_bits_per_symbol=1, frame_size=1024, channel="tdl", delay_spread=1e-10)
    ch.sample_channel_outputs(tf.zeros((200,1024,1),dtype=tf.int32),10.0)