...
class BinaryRNN(tf.keras.Model):
    ...
    def generate_binary_sequence(self, batch_size, sequence_length):
        # Start with an initial random binary input for the entire batch
        states = None
        generated_sequences = tf.TensorArray(dtype=tf.float32, size=sequence_length)
        generated_llrs = tf.TensorArray(dtype=tf.float32, size=sequence_length)
        ...
        return tf.transpose(generated_sequences.stack(), [1, 0, 2]), tf.transpose(generated_llrs.stack(), [1, 0, 2])