...
if isinstance(channel, ChannelMemory):
     PolarClass = SCTrellisDecoder
elif isinstance(channel, Channel) or isinstance(channel, OFDMSystem):
    PolarClass = SCDecoder
else: raise ValueError(f"invalid channel type: {channel}")
